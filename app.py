...
with exp1:
    st.download_button("📄 Generate Executive Pack PDF", data=_top_pdf, file_name="ecai_institutional_relationship_os_v10_0_1_alpha_executive_pack.pdf", mime="application/pdf", use_container_width=True)
with exp2:
    st.download_button("⬇️ Download MAS Scorecard CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="ecai_mas_v1_2_top10_relationships_v10_0_1_alpha.csv", mime="text/csv", use_container_width=True)
...
with tab_rel360:
    ...
    # Lightweight wallet-sizing proxy for v10.0.1. This becomes a full engine in v10.1.
    debt_b = safe_float(r360.get("Debt_B"), 0) or 0
    cash_b = safe_float(r360.get("Cash_B"), 0) or 0
    interest_b = safe_float(r360.get("InterestExpense_B"), 0) or 0
    ev_b = safe_float(r360.get("EV_B"), 0) or 0
    current_wallet_m = max(2, (interest_b * 8) + (cash_b * 0.03))
    estimated_wallet_m = max(current_wallet_m + 5, (debt_b * 0.08) + (cash_b * 0.05) + (ev_b * 0.01))
    wallet_gap_m = max(0, estimated_wallet_m - current_wallet_m)
    capture_rate = (current_wallet_m / estimated_wallet_m * 100) if estimated_wallet_m else 0
    ...
    <div class="rel360-wide-panel">
      ...
      <div class="ec-text">Early wallet-sizing proxy using debt, cash, interest expense and enterprise value. This becomes the full v10.1 Wallet Sizing Engine.</div>
      <div class="wallet-grid">
        <div class="wallet-mini"><div class="wallet-label">Current Wallet Proxy</div><div class="wallet-value">USD {current_wallet_m:,.1f}M</div><div class="wallet-sub">Interest + liquidity proxy</div></div>
        <div class="wallet-mini"><div class="wallet-label">Estimated Wallet</div><div class="wallet-value">USD {estimated_wallet_m:,.1f}M</div><div class="wallet-sub">Balance sheet opportunity</div></div>
...
with tab_memo:
    ...
    with c1:
        st.download_button("Generate Executive Pack PDF", data=pdf, file_name="ecai_institutional_relationship_os_v10_0_1_alpha_executive_pack.pdf", mime="application/pdf", use_container_width=True)
    with c2:
    ...
    with c3:
        st.download_button("Download Memo Text", data=memo_text.encode("utf-8"), file_name="ecai_v10_0_1_alpha_management_memo.txt", mime="text/plain", use_container_width=True)
