...
# EC-AI Institutional Relationship OS v9.2
# v9.2: Real Top 10 S&P universe + MAS v1.2 + MAS explainability + top executive pack export
# Run:
#   python -m streamlit run ecai_institutional_relationship_os_v9_2.py
...
st.set_page_config(
    page_title="EC-AI Institutional Relationship OS v9.2",
    page_icon="🏦",
    layout="wide",
)
...
st.markdown("""
<style>
.block-container { padding-top: 3.2rem; padding-left: 1.8rem; padding-right: 1.8rem; max-width: 1920px; }
...
.ec-title { font-size: 40px !important; font-weight: 950 !important; color:#071B3A; letter-spacing:-0.035em; line-height:1.28 !important; margin-top:10px; margin-bottom:8px; padding-top:4px; }
...
def build_portfolio_memo(data):
    ...
    lines.append("")
    lines.append("---")
    lines.append("Generated by EC-AI Institutional Relationship OS v9.2 | MAS v1.2")
    return "\n".join(lines)
...
def build_relationship_memo(row):
    ...
    lines.append("")
    lines.append("---")
    lines.append("Generated by EC-AI Institutional Relationship OS v9.2")
    return "\n".join(lines)
...
def build_executive_pack_pdf(data, selected_company=None):
    ...
    story.append(Spacer(1, 10))
    story.append(Paragraph("Generated by EC-AI Institutional Relationship OS v9.2 | Management Attention Allocation System", styles["ECSmall"]))
    doc.build(story)
    return buf.getvalue()
...
st.markdown("""
<div class="ec-hero">
  <div class="ec-title">EC-AI Institutional Relationship OS v9.2</div>
...
with exp1:
    st.download_button("📄 Generate Executive Pack PDF", data=_top_pdf, file_name="ecai_institutional_relationship_os_v9_2_executive_pack.pdf", mime="application/pdf", use_container_width=True)
with exp2:
    st.download_button("⬇️ Download MAS Scorecard CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="ecai_mas_v1_2_top10_relationships_v9_2.csv", mime="text/csv", use_container_width=True)
...
with tab_relationship:
    ...
    st.markdown(build_explainability_html(row), unsafe_allow_html=True)
...
with tab_reasoning:
    ...
    for _, r in df.iterrows():
        with st.expander(f"#{int(r['Rank'])} {r['Company']} · MAS {r['MAS']:.1f} · {r['Recommended_Action']}", expanded=(r['Rank'] <= 3)):
            ...
            """, unsafe_allow_html=True)
            st.markdown(build_explainability_html(r), unsafe_allow_html=True)
...
with tab_memo:
    ...
    with c1:
        st.download_button("Generate Executive Pack PDF", data=pdf, file_name="ecai_institutional_relationship_os_v9_2_executive_pack.pdf", mime="application/pdf", use_container_width=True)
    with c2:
    ...
    with c3:
        st.download_button("Download Memo Text", data=memo_text.encode("utf-8"), file_name="ecai_v9_2_management_memo.txt", mime="text/plain", use_container_width=True)
...
st.markdown("---")
st.caption("EC-AI Institutional Relationship OS v9.2 | Management Attention Allocation System (MAS) v1.2 | Real S&P Top 10 Public Company Universe")
