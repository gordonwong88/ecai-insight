# EC-AI Institutional Portfolio Dashboard v1.3
# Chart readability fix: no overlapping relationship names; numbered key labels + right-side reference table
# Run: python -m streamlit run ecai_institutional_portfolio_dashboard_v1_3.py
...
[data-testid="stSidebar"] { background: linear-gradient(180deg,#081B33 0%, #0C315A 100%); }
[data-testid="stSidebar"] * { color: white; }
.main-title { font-size: 46px; font-weight: 800; color: #071B3A; margin-bottom: 0px; }
.sub-title { color: #526173; font-size: 15px; margin-top: -6px; margin-bottom: 20px; }
.kpi-card { background: white; border: 1px solid #D8DEE6; border-radius: 14px; padding: 18px; box-shadow: 0 1px 3px rgba(15,23,42,0.06); }
.kpi-label { font-size: 13px; color: #526173; font-weight: 600; }
.kpi-value { font-size: 28px; color: #071B3A; font-weight: 850; margin-top: 8px; }
.kpi-sub { font-size: 12px; color: #7B8794; margin-top: 4px; }
.narrative-box { background: #F8FAFC; border-left: 5px solid #071B3A; border-radius: 12px; padding: 18px 22px; margin-top: 16px; margin-bottom: 18px; color: #071B3A; line-height: 1.6; font-size: 16px; }
.side-card { background: white; border: 1px solid #D8DEE6; border-radius: 14px; padding: 16px 18px; margin-bottom: 16px; box-shadow: 0 1px 3px rgba(15,23,42,0.05); }
.side-title { color: #071B3A; font-size: 22px; font-weight: 700; margin-bottom: 14px; }
...
label_mode = st.sidebar.radio("Chart labels", ["Number key labels", "Hide labels", "Show all names"], index=0)
...
st.markdown('<div class="sub-title">Executive view of institutional relationships | EC-AI Synthetic Institutional Portfolio Dataset v1.3</div>', unsafe_allow_html=True)
...
with left:
    ...
    st.markdown('<div class="small-note">X-axis: Treasury Score | Y-axis: Strategic Score | Bubble size: Exposure in USD billions. Names are moved to the reference table to avoid chart congestion.</div>', unsafe_allow_html=True)

    # Label only the most important names, using short numeric keys to avoid overlap.
    key_df = view.sort_values(["Exposure_USD_B", "Strategic_Score", "Risk_Score"], ascending=[False, False, False]).head(8).copy()
    key_map = {name: str(i + 1) for i, name in enumerate(key_df["Relationship"].tolist())}

    if label_mode == "Number key labels":
        view["Label"] = view["Relationship"].map(key_map).fillna("")
    elif label_mode == "Show all names":
    ...
    fig = px.scatter(
        ...
        hover_data={"Country": True, "Sector": True, "Exposure_USD_B": ":.1f", "Deposits_USD_B": ":.1f", "Risk_Score": True, "Label": False},
    )

    fig.add_shape(type="rect", x0=0, y0=70, x1=70, y1=100, fillcolor="rgba(255,152,0,0.10)", line_width=0)
    fig.add_shape(type="rect", x0=70, y0=70, x1=100, y1=100, fillcolor="rgba(76,175,80,0.10)", line_width=0)
    fig.add_shape(type="rect", x0=0, y0=0, x1=70, y1=70, fillcolor="rgba(244,67,54,0.06)", line_width=0)
    fig.add_shape(type="rect", x0=70, y0=0, x1=100, y1=70, fillcolor="rgba(33,150,243,0.06)", line_width=0)
    ...
    fig.add_annotation(x=23, y=96, text="<b>OPTIMIZATION FOCUS</b>", showarrow=False, font=dict(size=14, color="#6B4E16"))
    fig.add_annotation(x=88, y=96, text="<b>CROWN JEWEL</b>", showarrow=False, font=dict(size=14, color="#1B5E20"))
    fig.add_annotation(x=23, y=8, text="<b>PORTFOLIO REVIEW</b>", showarrow=False, font=dict(size=14, color="#7F1D1D"))
    fig.add_annotation(x=88, y=8, text="<b>TREASURY ANCHOR</b>", showarrow=False, font=dict(size=14, color="#0B3D75"))
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Treasury Score", range=[0, 100], dtick=10),
        yaxis=dict(title="Strategic Score", range=[0, 100], dtick=10),
    )

    st.plotly_chart(fig, use_container_width=True)

with right:
    st.markdown('<div class="side-title">Key Relationships</div>', unsafe_allow_html=True)
    key_df = view.sort_values(["Exposure_USD_B", "Strategic_Score", "Risk_Score"], ascending=[False, False, False]).head(8).copy()
    for i, (_, r) in enumerate(key_df.iterrows(), start=1):
        st.markdown(f"""<div class="side-card"><b>{i}. {r["Relationship"]}</b><br>{fmt_b(r["Exposure_USD_B"])} · {r["Priority"]}<br><span style="color:#526173;">T {int(r["Treasury_Score"])} | S {int(r["Strategic_Score"])} | Risk {int(r["Risk_Score"])}</span></div>""", unsafe_allow_html=True)
    ...
    ship_pct = view[view["Sector"].isin(["Shipping", "Aviation"])]["Exposure_USD_B"].sum() / total_exposure * 100
    st.markdown(f"""<div class="side-card"><b>Top 5 Relationships</b><br>{top5_pct:.1f}% of portfolio exposure</div>""", unsafe_allow_html=True)
    st.markdown(f"""<div class="side-card"><b>Infrastructure Concentration</b><br>{infra_pct:.1f}% of total exposure</div>""", unsafe_allow_html=True)
    st.markdown(f"""<div class="side-card"><b>Shipping & Aviation Risk</b><br>{ship_pct:.1f}% of exposure</div>""", unsafe_allow_html=True)
...
st.dataframe(
    attention[["Relationship", "Quadrant", "Priority", "Country", "Sector", "Exposure", "Deposits", "Treasury_Score", "Strategic_Score", "Risk_Score"]],
...
st.markdown("---")
st.caption("EC-AI Institutional Portfolio Prototype v1.3 | Chart Readability Fix")
