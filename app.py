...
# EC-AI Institutional Relationship OS v10.0 Alpha
...
st.set_page_config(
    page_title="EC-AI Institutional Relationship OS v10.0 Alpha",
    page_icon="🏦",
    layout="wide",
)
...
def build_portfolio_memo(data):
    ...
    lines.append("")
    lines.append("---")
    lines.append("Generated by EC-AI Institutional Relationship OS v10.0 Alpha | MAS v1.2")
    return "\n".join(lines)
...
def build_relationship_memo(row):
    ...
    lines.append("")
    lines.append("---")
    lines.append("Generated by EC-AI Institutional Relationship OS v10.0 Alpha")
    return "\n".join(lines)
...
def build_executive_pack_pdf(data, selected_company=None):
    ...
    story.append(Spacer(1, 10))
    story.append(Paragraph("Generated by EC-AI Institutional Relationship OS v10.0 Alpha | Management Attention Allocation System", styles["ECSmall"]))
    doc.build(story)
    return buf.getvalue()
...
# =========================
# Relationship 360 Helpers v10.0 Alpha
# =========================
...
st.sidebar.markdown("**v10.0 Alpha**")
st.sidebar.markdown("---")
...
st.markdown("""
<div class="ec-hero">
  <div class="ec-title">EC-AI Institutional Relationship OS v10.0 Alpha</div>
...
with exp1:
    st.download_button("📄 Generate Executive Pack PDF", data=_top_pdf, file_name="ecai_institutional_relationship_os_v10_0_alpha_executive_pack.pdf", mime="application/pdf", use_container_width=True)
with exp2:
    st.download_button("⬇️ Download MAS Scorecard CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="ecai_mas_v1_2_top10_relationships_v10_0_alpha.csv", mime="text/csv", use_container_width=True)

# =========================
# Tabs
# =========================
tab_queue, tab_command, tab_rel360, tab_execution, tab_portfolio, tab_actions, tab_relationship, tab_reasoning, tab_memo = st.tabs([
...
# =========================
with tab_rel360:
    section_title("Relationship 360 Intelligence", "The v10.0 flagship workspace: snapshot, timeline, network, product penetration and AI summary.")
    ...
    st.markdown(f"""
    <div class="rw-hero">
      <div class="rw-name">{r360['Company']}</div>
      <div class="rw-meta">{r360['Country']} · {r360['Sector']} · Rating {r360['Rating']} / {r360['Outlook']}</div>
    ...
    <div class="ec-kpi-row4">
      ...
      <div class="ec-card"><div class="ec-card-label">Debt</div><div class="ec-card-value">{fmt_b(r360['Debt_B'])}</div><div class="ec-card-sub">Wallet opportunity proxy</div></div>
    ...
    c_timeline, c_network = st.columns([1.15, 1], gap="large")
    with c_timeline:
        st.markdown('<div class="rel360-panel"><div class="rel360-panel-title">Relationship Timeline</div>', unsafe_allow_html=True)
    ...
    with c_network:
        ...
        st.markdown(f'<div class="rel360-panel"><div class="rel360-panel-title">Relationship Network</div><div class="ec-text">Coverage and product partners required to execute the recommended action.</div><br>{node_html}</div>', unsafe_allow_html=True)

    st.markdown('<div class="ec-table-title">Product Penetration & Wallet Opportunity</div>', unsafe_allow_html=True)
    ...
    c_ai, c_break = st.columns([1.2, 1], gap="large")
    with c_ai:
        st.markdown(f"""
        <div class="rel360-panel">
          <div class="rel360-panel-title">Relationship 360 AI Summary</div>
          <div class="ec-text">
            <b>{r360['Company']}</b> is classified as <b>{r360['MAS_Band']}</b> with MAS <b>{r360['MAS']:.1f}</b>.
            The primary driver is <b>{r360['Primary_Driver']}</b>. The recommended action is <b>{r360['Recommended_Action']}</b>.
            Based on S&P-derived scale, capital structure and external risk signals, management should use this page to align coverage, product partners and senior sponsorship around the next action.
            <br><br><b>Next 30 days:</b> confirm owner, validate wallet opportunity, agree executive follow-up and update execution status in the Management Execution Hub.
          </div>
        </div>
        """, unsafe_allow_html=True)
    with c_break:
        st.markdown('<div class="rel360-panel"><div class="rel360-panel-title">MAS Driver Breakdown</div>', unsafe_allow_html=True)
...
with tab_memo:
    ...
    with c1:
        st.download_button("Generate Executive Pack PDF", data=pdf, file_name="ecai_institutional_relationship_os_v10_0_alpha_executive_pack.pdf", mime="application/pdf", use_container_width=True)
    with c2:
    ...
    with c3:
        st.download_button("Download Memo Text", data=memo_text.encode("utf-8"), file_name="ecai_v10_0_alpha_management_memo.txt", mime="text/plain", use_container_width=True)
...
st.markdown("---")
st.caption("EC-AI Institutional Relationship OS v10.0 Alpha | Management Attention Allocation System (MAS) v1.2 | Real S&P Top 10 Public Company Universe")
