...
# v0.8.9.9 safe brand color constants
...
# EC-AI Banking Engine v0.8.9.9 - Readable dashboard with sidebar thresholds
...
def safe_wrap_label(value, width: int = 11) -> str:
    """Safely wrap chart labels for Plotly axes."""
    try:
    ...
    try:
        ...
        value = "N/A"
    try:
        return "<br>".join(textwrap.wrap(value, width=width)) if value else "N/A"
    except Exception:
        return value[:width] if value else "N/A"
...
# -----------------------------
st.set_page_config(
    page_title="EC-AI Banking Engine v0.8.9.9",
    layout="wide",
...
def combo_capital_fig(rel: pd.DataFrame, roe_floor: float) -> go.Figure:
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Lending Exposure (USD b)", showgrid=False, zeroline=False, range=[0, max(12, float(d["Lending_Drawn"].max())*1.28)], titlefont=dict(size=13), tickfont=dict(size=13)),
        yaxis2=dict(title="RoE %", overlaying="y", side="right", showgrid=False, zeroline=False, range=[10, 24], titlefont=dict(size=13), tickfont=dict(size=13)),
        xaxis=dict(tickangle=0, tickfont=dict(size=13), showgrid=False, automargin=True),
        hovermode="x unified",
    )
    return fig
...
with st.sidebar:
    ...
    st.markdown("<div class='sidebar-ver'>v0.8.9.9 Demo</div>", unsafe_allow_html=True)
...
def combo_capital_fig(rel: pd.DataFrame, roe_floor: float) -> go.Figure:
    """v0.8.9.9 executive-readable capital efficiency chart with larger labels."""
    d = rel.copy()
    ...
    d['Relationship_Wrapped'] = d['Relationship'].map(lambda v: safe_wrap_label(v, 11))
    ...
    fig.update_layout(
        title=dict(text='Capital Efficiency: Exposure vs LTM Group RoE', x=0.0, font=dict(size=17, color=NAVY)),
        template='plotly_white',
        height=520,
        margin=dict(l=68, r=70, t=72, b=120),
        paper_bgcolor='white',
        plot_bgcolor='white',
        font=dict(family='Inter, Arial', size=14, color=NAVY),
        legend=dict(orientation='h', y=1.08, x=0.03, font=dict(size=13, color=NAVY)),
        yaxis=dict(
            title='Lending Exposure (USD b)',
            showgrid=False,
            showline=False,
            zeroline=False,
            range=[0, max(float(exposure.max()) * 1.25, 1)],
            tickfont=dict(size=13, color='#50627A'),
            titlefont=dict(size=13, color='#50627A')
        ),
        yaxis2=dict(
            title='RoE %',
            overlaying='y',
            ...
            showline=False,
            zeroline=False,
            range=[max(0, min(float(roe.min()), roe_floor*100) - 2), max(float(roe.max()) + 3, roe_floor*100 + 3)],
            tickfont=dict(size=13, color='#50627A'),
            titlefont=dict(size=13, color='#50627A')
        ),
        xaxis=dict(
...
def combo_capital_fig(rel: pd.DataFrame, roe_floor: float) -> go.Figure:
    """v0.8.9.9 clean readable capital efficiency chart.
...
def remove_bar_value_labels(fig: go.Figure) -> go.Figure:
    """Remove dense bar value labels and keep hover only for cleaner executive charts."""
    try:
        fig.update_traces(text=None, texttemplate=None, textposition=None, selector=dict(type='bar'))
...
def render_revenue_exposure():
    ...
    # v0.8.9.9 revenue exposure label cleanup: value labels removed from dense product charts
    ...
    with c1:
        st.plotly_chart(remove_bar_value_labels(bar_fig(product, "Product_Type", "Revenue", "Revenue by Product Type", unit="M", height=430, width=0.36)), use_container_width=True, config={"displayModeBar": False}, key="rev_product_v099")
    with c2:
        st.plotly_chart(remove_bar_value_labels(bar_fig(product, "Product_Type", "Exposure", "Exposure by Product Type", unit="B", height=430, width=0.36)), use_container_width=True, config={"displayModeBar": False}, key="exp_product_v099")
...
st.markdown('''
<style>
.big-deposit-chart [data-testid="stVerticalBlock"] {
    gap: 0.35rem;
}
</style>
''', unsafe_allow_html=True)
...
def deposit_country_big_fig(df: pd.DataFrame) -> go.Figure:
    """Large, readable Deposits by Country chart for Deposit Intelligence."""
    d = df.copy()
    if "Country" not in d.columns:
        d["Country"] = "N/A"
    if "Deposit_Balance" not in d.columns:
        d["Deposit_Balance"] = 0

    d["Deposit_Balance"] = pd.to_numeric(d["Deposit_Balance"], errors="coerce").fillna(0)
    d = d.groupby("Country", as_index=False)["Deposit_Balance"].sum().sort_values("Deposit_Balance", ascending=False)

    colors = [globals().get("NAVY", "#0B1F3B"), globals().get("BLUE", "#2A5D9F"), "#7B8A97", "#94A1AD", "#AAB5BF", "#CBD3DA"]
    colors = (colors * ((len(d) // len(colors)) + 1))[:len(d)]
    ...
    fig = go.Figure(go.Bar(
        ...
        marker_color=colors,
        width=0.48,
        text=[f"${v:.1f}B" for v in d["Deposit_Balance"]],
        textposition="outside",
        cliponaxis=False,
        textfont=dict(size=24, color=globals().get("NAVY", "#0B1F3B"), family="Inter, Arial Black, Arial"),
        hovertemplate="%{x}<br>Deposits: $%{y:.1f}B<extra></extra>",
    ))

    ymax = float(d["Deposit_Balance"].max()) if len(d) else 1
    fig.update_layout(
        title=dict(text="Deposits by Country (USD b)", x=0.0, font=dict(size=20, color=globals().get("NAVY", "#0B1F3B"))),
        template="plotly_white",
        height=540,
        margin=dict(l=74, r=50, t=90, b=110),
        paper_bgcolor="white",
        plot_bgcolor="white",
        font=dict(family="Inter, Arial", size=16, color=globals().get("NAVY", "#0B1F3B")),
        showlegend=False,
        bargap=0.52,
    ...
    fig.update_yaxes(
        ...
        range=[0, ymax * 1.30 if ymax else 1],
        showgrid=False,
        showline=False,
        zeroline=False,
        tickfont=dict(size=16, color="#50627A"),
        titlefont=dict(size=16, color="#50627A"),
        automargin=True,
    )
    fig.update_xaxes(
        showgrid=False,
        showline=False,
        zeroline=False,
        tickfont=dict(size=16, color=globals().get("NAVY", "#0B1F3B")),
        tickangle=0,
        automargin=True,
    )
    return fig
...
def render_deposit_intelligence():
    ...
    # ROW 1: full-width / double-size Deposits by Country chart
    with st.container(border=True):
        st.plotly_chart(
            deposit_country_big_fig(country),
            use_container_width=True,
            config={"displayModeBar": False},
            key="dep_country_big_v0897"
        )

    # ROW 2: three supporting charts/cards
    c1, c2, c3 = st.columns([1.05, 1.05, 1.15], gap="large")
    with c1:
        with st.container(border=True):
            st.plotly_chart(
                donut_deposit(deposit),
                use_container_width=True,
                config={"displayModeBar": False},
                key="dep_type_donut_v0897"
            )
    ...
    with c2:
        ...
        <div class='small-card' style='min-height:330px;'>
            ...
            <div style='font-size:15px;color:#526173;line-height:1.45;margin:16px 0 20px 0;'>
                Deposit stability and funding resilience indicators.
            </div>
            <div style='display:flex;justify-content:space-between;margin:18px 0;font-size:16px;'><span>CASA Ratio</span><b style='color:#00875A;'>49%</b></div>
            <div style='display:flex;justify-content:space-between;margin:18px 0;font-size:16px;'><span>Loan to Deposit Ratio</span><b style='color:#D14343;'>53%</b></div>
            <div style='display:flex;justify-content:space-between;margin:18px 0;font-size:16px;'><span>NSFR</span><b style='color:#00875A;'>118%</b></div>
            <div style='display:flex;justify-content:space-between;margin:18px 0;font-size:16px;'><span>LCR</span><b style='color:#00875A;'>142%</b></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with c3:
        with st.container(border=True):
            st.plotly_chart(
                maturity_fig(maturity),
                use_container_width=True,
                config={"displayModeBar": False},
                key="dep_maturity_v0897"
            )

    # ROW 3: commentary table
...
# =============================================================
# v0.8.9.9 STRATEGY UPGRADE LAYER
...
def bar_fig(df: pd.DataFrame, x: str, y: str, title: str, unit: str = "M", height: int = 260, width: float = 0.34) -> go.Figure:
    """v0.8.9.9 upgraded bar chart: horizontal labels, larger values, cleaner executive display."""
    d = df.sort_values(y, ascending=False).copy()
    colors = [PALETTE[i] if i < len(PALETTE) else SLATE_2 for i in range(len(d))]
    suffix = "B" if unit == "B" else "M"
    fig = go.Figure()
...
# =============================================================
# v0.8.9.9 HOTFIX LAYER — readability, strategy and table fixes
# =============================================================
...
def bar_fig(df: pd.DataFrame, x: str, y: str, title: str, unit: str = "M", height: int = 260, width: float = 0.30) -> go.Figure:
    """Final v0.8.9.9 chart style: horizontal wrapped labels, larger values, wider chart margins."""
    d = df.sort_values(y, ascending=False).copy()
    colors = [PALETTE[i] if i < len(PALETTE) else SLATE_2 for i in range(len(d))]
    suffix = "B" if unit == "B" else "M"
...
def render_competitor_benchmarking():
    ...
    st.plotly_chart(bar_fig(comp, "Lead_Competitor", "Estimated_Wallet", "Estimated Wallet by Competitor Relationship", unit="M", height=460, width=0.34), use_container_width=True, key="competitor_wallet_v087_hotfix")
...
# =============================================================
# v0.8.9.9 FINAL OVERRIDES — readability + red-error fixes
# =============================================================
...
def style_banking_table(df: pd.DataFrame):
    """v0.8.9.9 safe table formatting — prevents red errors from mixed text/numeric fields."""
    d = df.copy()
    fmt = {}
    for c in d.columns:
        lc = str(c).lower()
...
def bar_fig(df: pd.DataFrame, x: str, y: str, title: str, unit: str = 'M', height: int = 380, width: float = 0.26) -> go.Figure:
    """v0.8.9.9: larger value labels + readable horizontal/wrapped x-axis + missing-column safe."""
    d = df.copy()
    if y not in d.columns:
        alias_map = {
            'Revenue': ['Total_Revenue', 'LTM_Revenue', 'Current_Revenue'],
            'Total_Revenue': ['Revenue', 'LTM_Revenue', 'Current_Revenue'],
            'Exposure': ['Lending_Drawn', 'Lending_Exposure', 'Total_Exposure'],
            'Lending_Drawn': ['Exposure', 'Lending_Exposure', 'Total_Exposure'],
            'Deposit_Balance': ['Deposits', 'Total_Deposits', 'Deposit Balance'],
        }
        for cand in alias_map.get(y, []):
...
def donut_deposit(deposit: pd.DataFrame) -> go.Figure:
    """v0.8.9.9 centered donut with annotation exactly at donut centre."""
    d = deposit.copy()
...
def render_executive_dashboard():
    ...
    st.markdown('<h1>Executive Portfolio Overview</h1>', unsafe_allow_html=True)
    st.markdown("<div class='subtitle'>LTM performance summary — wallet, exposure, deposits, revenue and profitability.</div>", unsafe_allow_html=True)
    total_rev = safe_sum(country, 'Total_Revenue') or safe_sum(country, 'Revenue')
    total_exp = safe_sum(country, 'Lending_Drawn') or safe_sum(country, 'Exposure')
    ...
    with c2:
        with st.container(border=True):
            st.plotly_chart(bar_fig(country, 'Country', 'Lending_Drawn', 'Exposure by Country (USD)', unit='B', height=390, width=0.34), use_container_width=True, config={'displayModeBar': False}, key='exec_exp_country_v0888')
    st.markdown('<h2>Deposit Intelligence</h2>', unsafe_allow_html=True)
    st.markdown("<div class='subtitle'>Franchise strength, liquidity profile and treasury opportunities.</div>", unsafe_allow_html=True)
    ...
    with d1:
        with st.container(border=True):
            ...
            dep_country_fig.update_traces(
                textfont=dict(size=20, color=globals().get('NAVY', '#0B1F3B'), family='Inter, Arial Black, Arial'),
                textposition='outside',
                cliponaxis=False,
                width=0.42
            )
            dep_country_fig.update_xaxes(
                ...
                tickangle=0,
                automargin=True,
                ticktext=['Hong Kong', 'Korea', 'Japan', 'Australia', 'Singapore', 'Taiwan']
            )
            dep_country_fig.update_layout(margin=dict(l=58, r=32, t=52, b=128), bargap=0.72)
            st.plotly_chart(dep_country_fig, use_container_width=True, config={'displayModeBar': False}, key='exec_dep_country_v0889')
    with d2:
        with st.container(border=True):
    ...
    st.markdown('<div style="height:16px"></div>', unsafe_allow_html=True)
    st.markdown("""
    <div class='insight-box' style='font-size:18px;line-height:1.78;padding:22px 26px;'>
      <b style='font-size:22px;'>Key Insights</b><br><br>
...
def maturity_fig(maturity: pd.DataFrame) -> go.Figure:
    """v0.8.9.9 ordered deposit maturity ladder. On Demand = immediately withdrawable / sight deposits."""
    d = maturity.copy()
...
def tenor_breakdown_fig(dsc: pd.DataFrame) -> go.Figure:
    """v0.8.9.9 ordered lending tenor breakdown: shortest tenor shown at top."""
    order = ["< 1 Year", "1 – 3 Years", "3 – 5 Years", "5 – 10 Years", "10+ Years"]
    d = dsc.copy()
    if "Tenor_Bucket" not in d.columns:
        rng = np.random.default_rng(11)
...
st.markdown("<div class='footer'>EC-AI Banking Intelligence Platform v0.8.9.9 · Demo data only · Do not use confidential bank data in public environments.</div>", unsafe_allow_html=True)
