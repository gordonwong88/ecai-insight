...
# EC-AI Institutional Relationship OS v9.2.3
# v9.2: Real Top 10 S&P universe + MAS v1.2 + MAS explainability + top executive pack export
# Run:
#   python -m streamlit run ecai_institutional_relationship_os_v9_2_2.py
...
st.set_page_config(
    page_title="EC-AI Institutional Relationship OS v9.2.3",
    page_icon="🏦",
    layout="wide",
)
...
def build_portfolio_memo(data):
    ...
    lines.append("")
    lines.append("---")
    lines.append("Generated by EC-AI Institutional Relationship OS v9.2.3 | MAS v1.2")
    return "\n".join(lines)
...
def build_relationship_memo(row):
    ...
    lines.append("")
    lines.append("---")
    lines.append("Generated by EC-AI Institutional Relationship OS v9.2.3")
    return "\n".join(lines)
...
def build_executive_pack_pdf(data, selected_company=None):
    ...
    story.append(Spacer(1, 10))
    story.append(Paragraph("Generated by EC-AI Institutional Relationship OS v9.2.3 | Management Attention Allocation System", styles["ECSmall"]))
    doc.build(story)
    return buf.getvalue()


# =========================
# v9.2.3 Visual / Explainability Helpers
# =========================
MCKINSEY_NAVY = "#071B3A"
# McKinsey-style executive palette: deep navy, muted blues, slate, amber.
# Avoid saturated consumer colors; charts should look like a board / consulting deck.
MCKINSEY_BLUE = "#1F5A93"
MCKINSEY_LIGHT_BLUE = "#A8C7E8"
MCKINSEY_STEEL = "#5B677A"
MCKINSEY_SLATE = "#8A94A6"
MCKINSEY_LIGHT = "#F3F6FA"
MCKINSEY_GRAY = "#D8DEE6"
MCKINSEY_AMBER = "#B7791F"
MCKINSEY_TEAL = "#2F7D7E"

MAS_BAND_COLORS = {
    "Executive Attention": MCKINSEY_NAVY,
    "Management Attention": MCKINSEY_BLUE,
    "Review": MCKINSEY_SLATE,
    "Monitor": "#B8C0CC",
}

ACTION_COLORS = {
    "Strategic Relationship Investment": MCKINSEY_NAVY,
    "Portfolio Monitoring": "#A7AFBD",
    "Relationship Recovery": MCKINSEY_TEAL,
    "Treasury Deep Dive": MCKINSEY_AMBER,
    "Cross-Border Expansion": MCKINSEY_STEEL,
    "Credit Review": "#8B2F2F",
    "Executive Engagement": "#32445C",
}
...
def apply_mckinsey_layout(fig, height=420, title=None):
    fig.update_layout(
        ...
        height=height,
        title=dict(text=title or fig.layout.title.text, x=0.0, xanchor="left", font=dict(size=17, color=MCKINSEY_NAVY)),
        font=dict(color=MCKINSEY_NAVY, size=12, family="Inter, Arial, sans-serif"),
        paper_bgcolor="white",
        plot_bgcolor="white",
        colorway=[MCKINSEY_NAVY, MCKINSEY_BLUE, MCKINSEY_SLATE, MCKINSEY_AMBER, MCKINSEY_TEAL, MCKINSEY_STEEL],
        legend=dict(title=None, orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1.0, font=dict(size=11, color=MCKINSEY_NAVY)),
        margin=dict(l=36, r=32, t=64, b=44),
    )
    fig.update_xaxes(showgrid=True, gridcolor="rgba(11,31,59,0.08)", zeroline=False, title_font=dict(color=MCKINSEY_STEEL), tickfont=dict(color=MCKINSEY_STEEL))
    fig.update_yaxes(showgrid=False, zeroline=False, title_font=dict(color=MCKINSEY_STEEL), tickfont=dict(color=MCKINSEY_STEEL))
    return fig


def build_explainability_html(row):
...
def safe_explainability_block(row):
    ...
    try:
        return build_explainability_html(row)
...
st.markdown("""
<div class="ec-hero">
  <div class="ec-title">EC-AI Institutional Relationship OS v9.2.3</div>
...
with exp1:
    st.download_button("📄 Generate Executive Pack PDF", data=_top_pdf, file_name="ecai_institutional_relationship_os_v9_2_3_executive_pack.pdf", mime="application/pdf", use_container_width=True)
with exp2:
    st.download_button("⬇️ Download MAS Scorecard CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="ecai_mas_v1_2_top10_relationships_v9_2_3.csv", mime="text/csv", use_container_width=True)
...
with tab_queue:
    ...
    with c1:
        plot = df.sort_values("MAS", ascending=True).copy()
        fig = go.Figure()
        fig.add_trace(go.Bar(
            x=plot["MAS"],
            y=plot["Company"],
            orientation="h",
            text=plot["MAS"].map(lambda x: f"{x:.1f}"),
            textposition="outside",
            marker=dict(color=[MAS_BAND_COLORS.get(x, MCKINSEY_SLATE) for x in plot["MAS_Band"]]),
            hovertemplate="%{y}<br>MAS %{x:.1f}<extra></extra>",
        ))
        apply_mckinsey_layout(fig, height=430, title="Management Attention Score by Relationship")
        fig.update_layout(showlegend=False, bargap=0.48, xaxis_title="MAS", yaxis_title="")
        fig.update_xaxes(range=[0, max(72, float(df["MAS"].max()) + 8)], showgrid=True)
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    with c2:
        action_mix = df["Recommended_Action"].value_counts().reset_index()
        action_mix.columns = ["Action", "Count"]
        fig2 = go.Figure(data=[go.Pie(
            labels=action_mix["Action"],
            values=action_mix["Count"],
            hole=0.55,
            textinfo="percent",
            marker=dict(colors=[ACTION_COLORS.get(a, MCKINSEY_SLATE) for a in action_mix["Action"]], line=dict(color="white", width=2)),
            sort=False,
        )])
        apply_mckinsey_layout(fig2, height=430, title="Action Mix")
...
with tab_portfolio:
    ...
    with c1:
        ...
        fig.update_traces(textposition="top center", marker=dict(line=dict(width=1.2, color="white"), opacity=0.86))
        apply_mckinsey_layout(fig, height=470)
        fig.update_layout(xaxis_title="Revenue (USD B)", yaxis_title="Debt (USD B)")
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    with c2:
        debt_plot = df.sort_values("Debt_B", ascending=False).copy()
        fig = go.Figure()
        fig.add_trace(go.Bar(
            x=debt_plot["Company"],
            y=debt_plot["Debt_B"],
            text=debt_plot["Debt_B"].map(lambda x: "" if pd.isna(x) else f"{x:.1f}B"),
            textposition="outside",
            marker=dict(color=[MCKINSEY_NAVY if i == 0 else MCKINSEY_BLUE for i in range(len(debt_plot))]),
            hovertemplate="%{x}<br>Debt %{y:.1f}B<extra></extra>",
        ))
        apply_mckinsey_layout(fig, height=470, title="Debt by Relationship")
        fig.update_layout(xaxis_title="", yaxis_title="Debt (USD B)", bargap=0.45, showlegend=False)
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
...
with tab_relationship:
    ...
    st.markdown(safe_explainability_block(row), unsafe_allow_html=True)
    ...
    st.markdown(f"<div class='memo-preview'>{build_relationship_memo(row).replace(chr(10), '<br>')}</div>", unsafe_allow_html=True)
    ...
    st.dataframe(breakdown, use_container_width=True, hide_index=True, height=220)

    rel_pdf = build_executive_pack_pdf(df, selected_company=selected)
    st.download_button("Download Relationship Executive Pack PDF", data=rel_pdf, file_name=f"ecai_{selected.lower().replace(' ', '_')}_executive_pack_v9_2_3.pdf", mime="application/pdf")
...
with tab_reasoning:
    ...
    for _, r in df.iterrows():
        with st.expander(f"#{int(r['Rank'])} {r['Company']} · MAS {r['MAS']:.1f} · {r['Recommended_Action']}", expanded=(r['Rank'] <= 3)):
            ...
            """, unsafe_allow_html=True)
            st.markdown(safe_explainability_block(r), unsafe_allow_html=True)
...
with tab_memo:
    ...
    with c1:
        st.download_button("Generate Executive Pack PDF", data=pdf, file_name="ecai_institutional_relationship_os_v9_2_3_executive_pack.pdf", mime="application/pdf", use_container_width=True)
    with c2:
    ...
    with c3:
        st.download_button("Download Memo Text", data=memo_text.encode("utf-8"), file_name="ecai_v9_2_3_management_memo.txt", mime="text/plain", use_container_width=True)
...
st.markdown("---")
st.caption("EC-AI Institutional Relationship OS v9.2.3 | Management Attention Allocation System (MAS) v1.2 | Real S&P Top 10 Public Company Universe")
