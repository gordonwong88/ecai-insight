...
# EC-AI Institutional Relationship OS v9.2.2
...
st.set_page_config(
    page_title="EC-AI Institutional Relationship OS v9.2.2",
    page_icon="🏦",
    layout="wide",
)
...
def build_portfolio_memo(data):
    ...
    lines.append("")
    lines.append("---")
    lines.append("Generated by EC-AI Institutional Relationship OS v9.2.2 | MAS v1.2")
    return "\n".join(lines)
...
def build_relationship_memo(row):
    ...
    lines.append("")
    lines.append("---")
    lines.append("Generated by EC-AI Institutional Relationship OS v9.2.2")
    return "\n".join(lines)
...
def build_executive_pack_pdf(data, selected_company=None):
    ...
    story.append(Spacer(1, 10))
    story.append(Paragraph("Generated by EC-AI Institutional Relationship OS v9.2.2 | Management Attention Allocation System", styles["ECSmall"]))
    doc.build(story)
    return buf.getvalue()
...
st.markdown("""
<div class="ec-hero">
  <div class="ec-title">EC-AI Institutional Relationship OS v9.2.2</div>
...
with exp1:
    st.download_button("📄 Generate Executive Pack PDF", data=_top_pdf, file_name="ecai_institutional_relationship_os_v9_2_1_executive_pack.pdf", mime="application/pdf", use_container_width=True)
with exp2:
...
with tab_queue:
    ...
    with c2:
        ...
        fig2 = px.pie(action_mix, values="Count", names="Action", title="Action Mix", color="Action", color_discrete_map=ACTION_COLORS)
...
with tab_memo:
    ...
    with c1:
        st.download_button("Generate Executive Pack PDF", data=pdf, file_name="ecai_institutional_relationship_os_v9_2_1_executive_pack.pdf", mime="application/pdf", use_container_width=True)
    with c2:
    ...
    with c3:
        st.download_button("Download Memo Text", data=memo_text.encode("utf-8"), file_name="ecai_v9_2_1_management_memo.txt", mime="text/plain", use_container_width=True)
...
st.markdown("---")
st.caption("EC-AI Institutional Relationship OS v9.2.2 | Management Attention Allocation System (MAS) v1.2 | Real S&P Top 10 Public Company Universe")
